...
const Help = () => {
  ...
  // ✅ FIXED: Proper typing for filtered FAQs
  const filteredFaqs: Record<string, FAQ[]> = searchQuery
    ? Object.entries(faqs).reduce((acc: Record<string, FAQ[]>, [category, questions]) => {
    ...
    <div className="space-y-8 max-w-6xl mx-auto p-6">
                ...
                <div key={category}>
                  ...
                  <Accordion type="single" collapsible className="space-y-2">
                    {questions.map((faq, index) => (
                      <AccordionItem key={`${category}-${index}`} value={`${category}-${index}`} className="border rounded-lg px-4">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Need personalized assistance? Our team is here to help!</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <a href="mailto:support@fasalguru.com" className="flex items-center gap-3 p-4 border rounded-lg hover:border-primary transition-colors">
      ...
      {/* Simplified remaining sections to avoid length issues */}
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
        ...
        <CardContent>
          <div className="grid gap-4 sm:grid-cols-2">
            {tutorials.map((tutorial, index) => (
              <div key={index} className="flex items-center gap-4 p-4 border rounded-lg hover:border-primary transition-colors cursor-pointer">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Select value={issueForm.type} onValueChange={(value) => setIssueForm({ ...issueForm, type: value as string })}>
              <SelectTrigger>
